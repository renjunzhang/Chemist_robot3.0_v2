...
class RobotMsgManager:
    ...
    def dms_callback(self, operate_datas):
        with self.callback_fun_mutex:
            ...
            # SPEED: 9.20演示，如果是充电指令，直接return, 不用打印
            if self.DMS_msg["operations"][0]["operation"] in ("charge", "uncharge"):
                return
            # SPEED: 9.20演示, 电化学3个站模拟放瓶子操作
            if self.DMS_msg["destination"] in (
                "furnace_workstation",
                "spotting_workstation",
                "confecting_workstation",
            ) and self.DMS_msg["operations"][0]["operation"] in (
                "put",
                "take",
                "take_bottle",
                "put_bottle",
                ...
                self.mqtt.MqttPublish(
                    ...
                    json.dumps(
                        {
                            "id": self.DMS_msg["id"],
                            "stamp": self.DMS_msg["stamp"],
                            "status": "",
                            "current_workstation": self.DMS_msg["destination"],
                            "state": "done",
                            "detail": {"navi": "", "oper": ""},
                        }
                    ),
