  ...
  if (argc == 1) {
    ...
    if (argc == 3) {
      amcl_node_ptr->runFromBag(argv[2]);
               ...
               (std::string(argv[3]) == "--global-localization")) {
...
AmclNode::AmclNode()
  ...
  if (private_nh_.hasParam("beam_skip_error_threshold_")) {
    ...
    private_nh_.param("beam_skip_error_threshold", beam_skip_error_threshold_,
                      0.9);
  }
  ...
  private_nh_.param("laser_likelihood_max_dist", laser_likelihood_max_dist_,
                    2.0);
...
void AmclNode::savePoseToServer() {
  ...
  // ROS_INFO("Saving pose to server. x: %.3f, y: %.3f",
  // map_pose.getOrigin().x(),
  //          map_pose.getOrigin().y());
...
bool AmclNode::getOdomPose(geometry_msgs::PoseStamped &odom_pose, double &x,
  ...
  // ROS_INFO("getOdomPose: [%.3f, %.3f, %.3f]", x, y, yaw);
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan) {
  ...
  // ROS_INFO("frame_to_laser_.size(): %d", frame_to_laser_.size());
  // ROS_INFO_STREAM("laser_scan_frame_id: " << laser_scan_frame_id);

  // 2.确认base_laser->base的坐标系转换关系
  // 先来一个条件判定: 如果当前缓存里没有雷达序号,找不到 base_laser->base
  // 的转换关系
  if (frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end()) {
    ...
    //新建一个 pf 位姿laser_pose_v，来接收laser_pose
    ...
    // 否则的话，就说明我们已经有了激光的位姿，获得激光的序号index:
    laser_index = frame_to_laser_[laser_scan_frame_id];
    // ROS_INFO_STREAM("\nretrieve laser index: " << laser_index);
  }
  ...
  // 先定义一个位姿pose, 这个 pose 是 odom 里程计数据
  pf_vector_t pose;
  ...
  // 这个 if 语句通过判断动的阈值判断机器人是否动了,然后决定是否更新
  // 这个是在雷达函数里的,每接收一帧激光点云, 算一下
  if (pf_init_) {
    // Compute change in pose
    // delta = pf_vector_coord_sub(pose, pf_odom_pose_);

    // 好好计算位姿的改变
    // pf_odom_pose_代表粒子滤波器里的里程计位姿，代表旧的位姿)
    // pose 为当前获取位姿
    // delta 为里程计获取的位姿改变量
    ...
    if (update) {
      ROS_INFO_STREAM("\033[1;33m Odom Changed!!!! \033[0m");
      ROS_INFO("\nlast_odom: [%.2f,%.2f,%.2f]", pf_odom_pose_.v[0],
               pf_odom_pose_.v[1], pf_odom_pose_.v[2]);
      ROS_INFO("\ncur_odom: [%.2f,%.2f,%.2f]", pose.v[0], pose.v[1], pose.v[2]);
      ROS_INFO("\nodom_delta: [%.2f,%.2f,%.2f]", delta.v[0], delta.v[1],
               delta.v[2]);
    }
  }
  ...
  //如果机器人已经动了，那就更新粒子滤波器
  // 关于lasers_update_[laser_index], 只要里程计动了且大于阈值,雷达就会更新
  // 如果里程计不动,雷达不可能更新
  else if (pf_init_ && lasers_update_[laser_index]) {
    // printf("pose\n");
    // pf_vector_fprintf(pose, stdout, "%.3f");

    // ROS_INFO("\n\033[1;33m In pf_init_ && lasers_update_[laser_index]
    // \033[0m");
    ...
    ROS_INFO_STREAM("\033[1;33m Update pf Odom Senser \033[0m");
  ...
  // 7.激光传感器更新符为真
  // 激光的数据data从激光传感器实际扫描到的距离数组送入观测模型，进行粒子滤波器的更新，
  //进一步进行重采样，重采样完了，将粒子滤波器的粒子sample集取出来，作为粒子云发布出去。
  ...
  if (lasers_update_[laser_index]) {
    ...
    ROS_INFO_STREAM("\033[1;32m<---Robot moved!!!--->\033[0m");

    //创建激光雷达数据为ldata
    AMCLLaserData ldata;

    //用laser[laser_index]填充ldata.sensor
    ...
    if (ldata.range_max <= 0.0 || range_min < 0.0) {
      ROS_ERROR("range_max or range_min from laser is negative! ignore this "
                "message.");
      return; // ignore this.
    }
    ...
    for (int i = 0; i < ldata.range_count; i++) {
      ...
      if (laser_scan->ranges[i] <= range_min)
        ...
        ldata.ranges[i][0] =
            std::numeric_limits<decltype(ldata.range_max)>::max();
      else
    ...
    ROS_INFO_STREAM("\033[1;33m Update pf Laser Senser \033[0m");

    // 更新完观测模型,雷达更新标志位置零,等待下一次里程计改动
    ...
    if (!(++resample_count_ % resample_interval_)) {
      ...
      //这里使用的是pf_update_resample(pf_);也就是KLD采样算法
      //将已重采样标识符置为true
  ...
  // 8.如果已经重采样或者强制发布
  //已经重采样后或者是强制发布，
  //这时候就要发布AMCL包经历粒子滤波算法估计出的世界坐标系下的机器人位姿p。
  //如何选出呢？选出拥有最大权重的粒子sample簇，
  // 并将这个簇的均值赋给位姿p的位姿x,y,yaw，将粒子sample集的协方差赋给位姿p的协方差。
  if (resampled || force_publication) {
    // ROS_INFO_STREAM("\n\033[1;33m In resampled || force_publication
    // !\033[0m"); Read out the current hypotheses
    ...
    if (max_weight > 0.0) {
      ...
      ROS_INFO("Max weight: %.3f, Max weight pose: %.3f %.3f %.3f", max_weight,
               hyps[max_weight_hyp].pf_pose_mean.v[0],
               hyps[max_weight_hyp].pf_pose_mean.v[1],
               hyps[max_weight_hyp].pf_pose_mean.v[2]);
  ...
  } else if (latest_tf_valid_) { // 9.如果最新的tf存在的话
    if (tf_broadcast_ == true) {
