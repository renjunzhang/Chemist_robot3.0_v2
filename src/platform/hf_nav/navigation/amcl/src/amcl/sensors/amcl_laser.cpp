...
////////////////////////////////////////////////////////////////////////////////
// Apply the laser sensor model 设置激光观测模型
...
double AMCLLaser::BeamModel(AMCLLaserData *data, pf_sample_set_t *set) {
  ...
  for (j = 0; j < set->sample_count; j++) {
    ...
    // ROS_INFO("\namcl_laser model: %d\nstep: data->range_count: "
    //          "%d\nself->max_beams: %d",
    //          step, data->range_count, self->max_beams);
...
double AMCLLaser::LikelihoodFieldModelProb(AMCLLaserData *data,
  ...
  // Beam skipping - ignores beams for which a majority of particles do not
