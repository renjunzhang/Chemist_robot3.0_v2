...
// sensor文件夹主要表达了两个模型，
// 一个是运动模型，根据机器人运动类型，选择相应的运动模型，
// 用来将粒子一起运动(平移，旋转)，得到粒子样本的位姿；
// 另一个是观测模型，根据选定的观测更新模型，对给定的位姿计算权重。

// 在amcl_sensor.h定义基类AMCLSensor 和基类AMCLSensorData,
// 为两个派生类amcl_laser和amcl_odom提供统一接口。

// 先来看AMCLSensor类，主要功能是根据运动模型更新粒子滤波器，
// 根据观测模型初始化滤波器，根据观测模型更新粒子滤波器，仅定义运动模型里的位姿。
// 从这里可以看到，运动模型和观测模型与粒子滤波器联系紧密。
