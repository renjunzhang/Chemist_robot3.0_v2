...
class PlatformCommunication:
    def __init__(
        ...
        self.mqtt_port = 1883
        self.client = mqtt.Client("robot_client")
        ...
        self.session_gstatus = requests.Session()
        self.session_gstatus.cookies.set("sender", self.robot_name)
        self.session_amclpose = requests.Session()
        self.session_amclpose.cookies.set("sender", self.robot_name)
        ...
        # 这个线程用来接收全局机器人状态
        self.thread3 = threading.Thread(target=self.get_global_status)
        self.thread3.start()
    ...
    def amcl_pose_callback(self, pos):
        ...
        try:
            response = self.session_amclpose.put(
                self.host_ip + "/mapf_pose",
                headers={"Content-Type": "application/json"},
                data=json.dumps(pose_dict),
            )
            # rospy.loginfo("Send Pose: x: %f y: %f", pose.position.x, pose.position.y)
        except:
            rospy.logwarn("FUNC callback: 无法连接flask_core服务器")
    ...
    def get_global_status(self):
        while not rospy.is_shutdown():
            try:
                response = self.session_gstatus.get(self.host_ip + "/mapf_status")
                self.global_status = json.loads(response.text)
                rospy.sleep(0.1)
            except:
                rospy.logwarn("FUNC get_global_status: 无法连接flask_core服务器")
                rospy.sleep(0.1)
                continue
