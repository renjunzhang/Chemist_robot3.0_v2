...
//  return {{r, g, b}};
    ...
    // 根据packed获取像素值[0-255]
    const unsigned char tmp_color = color >> 16;
    // 根据packed获取这个栅格是否被更新过
    const unsigned char observed = color >> 8;

    uint8 value = 205;
    if (observed != 0)
    {
        int value_temp = ::cartographer::common::RoundToInt((1. - tmp_color / 255.) * 100.);
        if (value_temp > 100 * 0.5)
            value_temp = 0;
