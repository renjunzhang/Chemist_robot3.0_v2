...
DEFINE_string(configuration_directory,
              "/home/shao/sourcecode/c++/clion_carto_ws/src/cartographer_ros/"
              "cartographer_ros/configuration_files",
              "First directory in which configuration files are searched, "
              "second is always the Cartographer installation to allow "
              "including files from there.");
DEFINE_string(configuration_basename, "my_carto.lua",
              "Basename, i.e. not containing any directory prefix, of the "
              "configuration file.");
DEFINE_string(
    load_state_filename,
    "/home/shao/sourcecode/c++/aichem_cartographer/src/cartographer_ros/"
    "cartographer_ros/configuration_files/aichem_map_back.pbstream",
...
/* 像move_base中的amcl模块一样，通过订阅/initialpose话题达到修正定位结果的目的
void Reset_InitPose_callback(const
...
template <typename T>
T getParam_Func(ros::NodeHandle& pnh, const std::string& param_name,
                const T& default_val) {
  T param_val;
  bool get_scuess = pnh.param<T>(param_name, param_val, default_val);
  if (!get_scuess) {
    LOG(ERROR) << "get " << param_name << " error";
  }
...
void Run() {
  ...
  // 获取轨迹配置项指针
  auto trajectory_options_handle =
      &(trajectory_options);  // 原博文这里少了个auto
  ros::NodeHandle pnh;
  // 获取配置参数
  bool localization =
      getParam_Func<bool>(pnh, "localization", true);  // 是否为纯定位模式
  double pos_x = getParam_Func<double>(pnh, "initial_pose_x", 0.0);
  double pos_y = getParam_Func<double>(pnh, "initial_pose_y", 0.0);
  double pos_z = getParam_Func<double>(pnh, "initial_pose_z", 0.0);
  double pos_ox = getParam_Func<double>(pnh, "initial_pose_ox", 0.0);
  double pos_oy = getParam_Func<double>(pnh, "initial_pose_oy", 0.0);
  double pos_oz = getParam_Func<double>(pnh, "initial_pose_oz", 0.0);
  double pos_ow = getParam_Func<double>(pnh, "initial_pose_ow", 1.0);
  ...
  printf("pos_x:%lf, pos_y:%lf", pos_x, pos_y);
  //   ROS_INFO("NodeHandle Namespace: %s", pnh.getNamespace().c_str());
  //   ROS_INFO("Full Parameter Name: %s",
  //   pnh.resolveName("initial_pose_x").c_str());

  if (localization == true) {
    LOG(WARNING) << "localization mode";

    // 更改轨迹配置项中的初始位姿值,
    // 在map_build的AddTrajectoryBuilder中调用SetInitialTrajectoryPose,最后赋值给后端的data_.initial_trajectory_poses
    *trajectory_options_handle->trajectory_builder_options
         .mutable_initial_trajectory_pose()
         ->mutable_relative_pose() = cartographer::transform::ToProto(
        cartographer_ros::ToRigid3d(init_pose));
    if (!FLAGS_load_state_filename.empty()) {
      node.LoadState(FLAGS_load_state_filename, FLAGS_load_frozen_state);
    }
  } else {
    LOG(WARNING) << "carto mode";
  }
